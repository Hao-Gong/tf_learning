...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(192 * 4 * 4, 20000)
        self.fc2 = nn.Linear(20000, 200)
...
for epoch in range(EPOCH):
    ...
    for i_test, (x_test, y_test) in enumerate(test_loader):
        # volatile=True close the backpropogation
        x_t = Variable(x_test.cuda(), volatile=True)
        y_t = Variable(y_test.cuda())
        test_output = cnn(x_t)
        pred = test_output.data.max(1, keepdim=True)[1]  # get the index of the max log-probability
        correct = pred.eq(y_t.data.view_as(pred)).cpu().sum()/TEST_BATCH_SIZE
        print('Epoch: ', epoch, '| train loss: %.4f' % loss.data[0], '| test accuracy: %.2f' % correct)
        break